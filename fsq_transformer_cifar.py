...
class FSQ_Transformer(nn.Module):
    ...
    def quantize(self, z_e): # z_e.shape: [b * max_seq_len, latent_dim]
        L = torch.tensor(self.L).unsqueeze(0).to(device) # [1, latent_dim]
...
def cifar10_dl(img_size, batch_size):
    ...
    dataset = CIFAR10(
        "/home/vivswan/experiments/consistency_models/dataset_cifar",
        train=True,
        download=True,
        transform=tf,
    )
...
if __name__ == '__main__':
    ...
    random_seed = 101010 # 101010

    checkpoint_path = f'./ckpts/FSQ_Transformer_cifar|patchSize={patch_size}.pt' # path to a save and load checkpoint of the trained model
    resume_training_from_ckpt = True              

    save_img_folder = f'./out_imgs_FSQ_Transformer_cifar|patchSize={patch_size}'
    ...
    # train 
    for epoch in tqdm(range(epochs_done, epochs_done + max_epochs)):
